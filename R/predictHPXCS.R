...
#' @export
#'
#' @examples cluster <- predictHPXCS(test)
predictHPXCS <- function(blcadata){
   library(openxlsx)
   library(Matrix)
   library(impute)
   library(limma)
  ...
  xgbpre <- xgboost( data = dtrain2,max_depth=5  #树的深度自己
                     ...
                     , objective='binary:logitraw' #二分类，线性回 需要修
                     ,nround=100)
  ...
  ##输出
 pre_test2 <- pre_test
 pre_test2[pre_test2 >= 0] <- "B"
 pre_test2[pre_test2 < 0] <- "A"




 out <- cbind(ID=rownames(data2),Cluster=pre_test2)
 out <- cbind(out, Score=pre_test)
